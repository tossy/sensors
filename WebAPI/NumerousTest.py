...
class NumerousTest:
    ...
    def updateMetricPhoto(self, metricID, filepath):
        ...
        url = "https://api.numerousapp.com/v2/metrics/" + metricID + '/photo'
        ...
        authTuple = (self.key, '')
        #mpart = {'image':('image.img', open(filepath, 'rb'))}
        print(mpart)
        #req = urllib.request.Request(url, data=file, headers=self.header)
        #res = urllib.request.urlopen(req)
        res = requests.request('POST', url, auth=authTuple, data=None, files=file, headers=None)
        #res = requests.post(url, files=mpart, headers=self.header)
        print(res)
